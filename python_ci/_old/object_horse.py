...
class Horse:
    ...
    def setTrainerAndHorseOwner(self):
        url = self.getHorseURL(self.horse_num)
    ...
    def isValid(self, index):
        if (
            self.race_results_data_frame.iat[index, 23] != "計不"
            and self.race_results_data_frame.iat[index, 11] != "除"
        ):
            return True
        else:
            return False
