...
class Race:
    ...
    def adddfdatetolist(label, dataframe):
        list = []
    ...
    def getmoneyinrace(label, dataframe):
        money = dataframe[label][0]
        if math.isnan(money):
            money = 0.0
        return money

    def gettrackcondition(label, dataframe):
        dirt_or_grass = dataframe[label][0]
        print(dirt_or_grass)
        condition_mapping = {"良": 4, "稍": 3, "重": 2, "不": 1}
        tmp_condition = condition_mapping[dirt_or_grass]
    ...
    def addweathertolist(label, dataframe):
        ...
        return weather_mapping[dataframe[label][0]]

    def addgrassordirttolist(label, dataframe):
        dirt_mapping = {"芝": 1, "ダ": 2, "障": 3}
        print(dataframe[label][0][0:1])
        return dirt_mapping[dataframe[label][0][0:1]]
    ...
    def addgraderaceinfotolist(label, dataframe):
        ...
        for racename in dataframe[label]:
            if "(G3)" in dataframe[label]:
                if "ユニコーン" in dataframe[label] or "レパード" in dataframe[label]:
                    g3_age3 = 1
                    g3_age4 = 0
                else:
                    g3_age3 = 0
            ...
            if "(G2)" in dataframe[label]:
                if "関東オークス" in dataframe[label] or "兵庫チャンピョン" in dataframe[label]:
                    g2_age3 = 1
                    g2_age4 = 0
                else:
                    g2_age3 = 0
            ...
            if "(G1)" in dataframe[label]:
                if "ジャパンダートダ" in dataframe[label]:
                    g1_age3 = 1
                    g1_age4 = 0
                else:
                    g1_age3 = 0
    ...
    def makeRaceInfo(self, day, racename):
        df = pd.DataFrame()
        OBJECT_DAY = day
        RACE_NAME = racename
        ...
        for horse_id in horse_id_list:
            ...
            horse_info_path_tmp = (
                "./python_ci/csv/scrape/horse_id/" + str(horse_id) + ".csv"
            )
            is_file = os.path.isfile(horse_info_path_tmp)
            if is_file == False:
                horse_info_data_frame_tmp = getHorseInfo(horse_id)
                print(horse_info_data_frame_tmp)
                horse_info_data_frame_tmp.to_csv(horse_info_path)
            else:
                horse_info_data_frame_tmp = pd.read_csv(
                    horse_info_path_tmp, index_col=0
                )

            date = self.adddfdatetolist("日付", horse_info_data_frame_tmp)
            ...
            for num in range(len(date)):
                ...
                horse_id_set = self.addhorseidtolist(
                    horse_id, horse_info_data_frame_tmp[0]
                )

                print(horse_info_data_frame[0:1])

                tmp_data = self.adddfdatatolistadddfdatatolist(
                    "開催", horse_info_data_frame[num : num + 1]
                )
                place.append(tmp_data)

                same_track.append(
                    self.addsametracktolist(
                        "開催", horse_info_data_frame[num : num + 1], race
                    )
                )

                race_num.append(
                    self.adddfdatatolist("R", horse_info_data_frame[num : num + 1])
                )

                horse_num.append(
                    self.adddfdatatolist("馬番", horse_info_data_frame[num : num + 1])
                )

                horse_race_anim.append(
                    self.adddfdatatolistadddfdatatolist(
                        "頭数", horse_info_data_frame[num : num + 1]
                    )
                )

                weather.append(
                    self.addweathertolist("天気", horse_info_data_frame[num : num + 1])
                )

                dirt_grass.append(
                    self.addgrassordirttolist(
                        "距離", horse_info_data_frame[num : num + 1]
                    )
                )

                distance.append(
                    self.adddistancetolist("距離", horse_info_data_frame[num : num + 1])
                )

                condition.append(
                    self.addtrackcondition("馬場", horse_info_data_frame[num : num + 1])
                )

                furlong3.append(
                    self.adddfdatatolist("上り", horse_info_data_frame[num : num + 1])
                )

                money = self.addmoneytolist("賞金", horse_info_data_frame[num : num + 1])
                ...
                (
                    ...
                    g3_age3_tmp,
                    g3_age4_tmp,
                ) = self.addgraderaceinfotolist("レース名", horse_info_data_frame)

                g1_age3.append(g1_age3_tmp)
                g2_age3.append(g2_age3_tmp)
                g3_age3.append(g3_age3_tmp)
                g1_age4.append(g1_age4_tmp)
                g2_age4.append(g2_age4_tmp)
                g3_age4.append(g3_age4_tmp)
                ...
                if tmp_racetrack > 10 and tmp_racetrack <= 100:
                    ...
                    race_id = (
                        tmp[0] + str(tmp_day) + tmp[1] + tmp[2] + tmp_race
                    )  # 年+開催+月日
                else:
                    tmp_day = (
                        str(tmp_racetrack)
                ...
                if hasraceinfo == False:
                    ...
                    tmp2_condition = (
                        str(int(tmp_time[0]) * 60 + int(tmp_time[2:4]))
                        + "."
                        + tmp_time[5]
                    )
        ...
        if teiou_test_2023_df.empty != True:
            horse_info_new_path = (
                "./python_ci/csv/learning/" + RACE_NAME + "_result.csv"
            )
