    ...
    for hogehoge in range(0,2):
        ...
        for jockey_id in range(counter_start,counter_end):
            for counter in range(0,2):
                if counter == 0:
                    url ='https://db.netkeiba.com/jockey/result/recent/' +"0"+ str(jockey_id)
                else:
                    url ='https://db.netkeiba.com//?pid=jockey_detail&id=0'+ str(jockey_id) + '&page=2'

                if hogehoge != 1:
                    if counter == 0:
                        url ='https://db.netkeiba.com/jockey/result/recent/' + hex(jockey_id).replace('0x','')
                    else:
                        url ='https://db.netkeiba.com//?pid=jockey_detail&id=0'+ hex(jockey_id).replace('0x','') + '&page=2'
